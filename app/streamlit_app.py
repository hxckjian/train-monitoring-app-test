...
def run_panel(spec: SubsystemSpec, button: str):
    ...
    if st.button(button, type="primary", disabled=payload is None, key=f"{spec.key}_run"):
        try:
            if spec.key == "shm":
                bar = st.progress(0.0, text="Counting stress cycles...")

                def tick(i, n, name):
                    bar.progress(i / n, text=f"Rainflow-counted {name}  ({i}/{n})")
    ...
    for col, spec in zip(cols, SUBSYSTEMS.values()):
        ...
        with col:
            if card:
                ...
                mean = v.get("mean_iou_weighted_f1", v.get("mean_score"))
                std = v.get("std_iou_weighted_f1", v.get("std_score"))
                metric = ("IoU-weighted F1" if spec.key == "door" else "max(0, 1 − MAPE)")
...
def rail_page() -> None:
    pending_page("rail", [
        "272 training recordings: 234 Normal, 14 Side I, 24 Side II. Macro F1 means each "
        "rare Side I file is worth roughly a twentieth of the subsystem score.",
        "Each file is 1 second at 10 kHz: 64 axle boxes × (vibration, shock) plus rotating speed.",
        "Positions 1, 3, 5, 7 sit on the Side I rail; 2, 4, 6, 8 on Side II.",
        "Corrugation is a spatial wavelength, so the spectrum must be converted with "
        "λ = v / f using the measured speed (90-tooth wheel, 0.85 m diameter).",
    ])


def acv_page() -> None:
    pending_page("acv", [
        "Six training cases, one faulty car each; one test case. A random ranking already "
        "scores 0.5625, so the honest headroom is about 0.44.",
        "Column order is scrambled differently in every file, and only one parameter name "
        "is shared by all seven - parsing must go by normalised name.",
        "Case 04 carries 63 parameters per car and has no 'Indoor Average Temperature'.",
        "Faulty cars in training are 01, 02, 03, 01, 04, 06 - any rule that favours low car "
        "numbers is fitting the answers, not the physics.",
    ])
...
digraph G {
  ...
  rail [label="subsystems/rail\\n(pending)", fontcolor="#626c7a", style="rounded,dashed"];
  ...
  door -> art; shm -> art; ui -> sub; sub -> zip; score -> door [style=dashed, label=" validates", fontsize=9, fontcolor="#626c7a"];
...
- **Nothing is shown that was not validated.** Pending subsystems say so instead of guessing.
